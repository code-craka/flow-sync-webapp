...
import { Zap, Mail, Lock, Eye, EyeOff, Github, Info } from 'lucide-react'; // Added Github, Info, Eye, EyeOff
...
const SignInPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast({ title: "Signed in successfully!", description: "Welcome back to FlowSync AI.", className: "bg-card text-card-foreground rounded-xl shadow-lg" });
  ...
  const handleOAuthSignIn = async (provider) => {
    ...
    try {
      ...
      if (error) throw error;
      // Supabase handles redirection, user will be redirected by Supabase
    } catch (error) {
      toast({
  ...
  const handleMagicLinkSignIn = async (e) => {
    e.preventDefault();
    if (!magicLinkEmail) {
      toast({ title: "Email is required", description: "Please enter your email address for the magic link.", variant: "destructive" });
      return;
    }
    ...
    try {
      const { error } = await signInWithOtp({ 
        ...
        options: {
          emailRedirectTo: `${window.location.origin}/auth/callback` // Or directly to /app/dashboard if handled well
        }
      });
      if (error) throw error;
      setModalContent({
        title: 'Magic Link Sent!',
        description: `A sign-in link has been sent to ${magicLinkEmail}. Check your inbox (and spam folder) to complete the process.`
      });
      setIsModalOpen(true);
      setMagicLinkEmail(''); // Clear input after sending
    } catch (error) {
      toast({
        title: "Magic link failed",
        description: error.message || "Could not send the magic link. Please try again.",
        variant: "destructive",
      });
    } finally {
  ...
  return (
    ...
    >
      <Card className="w-full shadow-2xl rounded-2xl border-border overflow-hidden">
        <CardHeader className="text-center p-6 sm:p-8 bg-muted/30">
          ...
          <CardDescription className="text-muted-foreground text-sm sm:text-base">Sign in to continue to FlowSync AI.</CardDescription>
        </CardHeader>
        <CardContent className="px-6 sm:px-8 py-6 space-y-6">
          ...
          <form onSubmit={handleMagicLinkSignIn} className="space-y-3 pt-2">
             <div className="space-y-1.5">
            ...
            <Button type="submit" variant="secondary" className="w-full h-11 sm:h-12 text-base rounded-lg" disabled={magicLinkLoading || loading}>
    ...
    <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-[425px] rounded-xl shadow-2xl border-border">
          <DialogHeader className="p-6 text-center">
            <Info className="h-12 w-12 text-primary mx-auto mb-4"/>
            <DialogTitle className="text-2xl font-bold gradient-text">{modalContent.title}</DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter className="p-6 pt-0">
            <Button onClick={() => setIsModalOpen(false)} className="w-full rounded-lg">Got it!</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
